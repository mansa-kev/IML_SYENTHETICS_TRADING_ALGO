    ...
    <div className="min-h-screen bg-[#111513] text-slate-100 font-sans antialiased flex flex-col p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <section className="w-full max-w-none bg-[#101512] border border-brand-teal/25 rounded-xl p-3 flex flex-wrap items-center gap-2 shadow-lg">
        ...
        {[
          ...
          <button
            key={range}
            onClick={() => window.open(`/api/logs/export?${new URLSearchParams({ range })}`, "_blank")}
        ...
        />
        <button
          onClick={() => window.open(`/api/logs/export?${new URLSearchParams({ ...(logStartDate ? { start: logStartDate } : {}), ...(logEndDate ? { end: logEndDate } : {}) })}`, "_blank")}
